...
import { useGetNewsCategoriesQuery } from "@/redux/services/news/newsCalssificationApi";
...
import { useAddNewsCategoryMutation } from "@/redux/services/news/newsCalssificationApi";
...
export default function Categories() {
  ...
  const selectedCat = useAppSelector(selectedNewCategory)
  const showToast = useToast();
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
  const { hasPermission } = usePermission();
  const { data: categories, isLoading, isSuccess } = useGetNewsCategoriesQuery(9);
  const [addPaparentCategory, setAddParentCategory] = useState<AddCategories>({
    parent_id: 0,
    title: "",
  });
  ...
  const handleAddCategory = () => {
    addNewsCategory(addPaparentCategory);
  };

  const handleCategorySelect = (node: Category) => {
    dispatch(setSelectedNewCategory(node))
     ...
     { hasPermission("Classification.create")  && <ParentMenu
        ...
        >
          <Stack direction="row" spacing={3} p={1}>
            <TextField
              value={addPaparentCategory.title}
