...
import { useUpdateNewsCategoryMutation,
useAddNewsCategoriesMutation,
useDeleteNewsCategoryMutation
} from "@/redux/services/news/newsCalssificationApi";
...
} from "@/redux/features/news/newsClassificationSlice";
...
const AddEditCategory: React.FC<SharedCategory> = () => {
  ...
  const handleSave = (): void => {
    addNewsCategories(additionArray);
  };
  ...
  const handleUpdate = (e: React.MouseEvent<HTMLButtonElement>, id: number) => {
    updateNewsCategory({ parent_id: id, title: rename });
    setRename("");
  };
  ...
  const handleDelete = (id: number): void => {
    ...
    if (deleteStatus) {
      setSelectedNewCategory({ ...selectedCat, id: 0 });
    }
  };
  ...
  useEffect(() => {
    if (deleteStatus) {
      ...
      deleteTimeoutRef.current = setTimeout(() => {
        dispatch(setSelectedNewCategory({}));
        setAdditionArray([]);
      }, 2000);
    }
    setAdditionArray([]);
    ...
    <Box display="flex" flexDirection="column" gap="20px" alignItems="center">
      ...
      <Stack direction="row" spacing={2}>
        ...
        {hasPermission("Classification.delete") && (
          <Tooltip title="حذف کامل دسته با زیر دسته های آن">
            <LoadingButton
              size="small"
              color="error"
