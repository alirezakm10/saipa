...
import { useGetSlidersQuery, useDeleteSliderMutation, useSetDefaultSliderMutation } from "@/redux/services/other/slidersApi";
...
const BannerSliderList: React.FC<Props> = ({ type }) => {
  const showToast = useToast();
  const { hasPermission } = usePermission();
  const [itemId, setItemId] = useState<number>(0)

  const { data: sliders, isSuccess, isLoading, isFetching, isError, error : slidersError } = useGetSlidersQuery({ type })
  const [
    deleteSlider,
  ...
  const [
    setDefaultSlider,
    { isSuccess: setDefaultStatus, isLoading: setDefaultLoader, data: setDefaultResult },
...
const handleSetDefault = (id:number) => {
    ...
    hasPermission("Promotion.edit") &&  hasPermission("Promotion.delete") ? {
      ...
      renderCell: (slider : any) => (
        <Stack direction="row" spacing={2}>
         { hasPermission("Promotion.edit") &&  <Link href={`/adminpanel/other/sliders/update-slider/${slider.row.id}`}>
            <Button
              variant="outlined"
              color="warning"
         ...
         { hasPermission("Promotion.delete") && <ConfirmModal
  ...
  function CustomToolbar() {
    return (
      <GridToolbarContainer sx={{ justifyContent: 'space-between' }}>
        {hasPermission("Promotion.create") && <Link href='/adminpanel/other/sliders/add-main-slider/1' >
          <Button
            variant='outlined'
            color='primary'
  ...
  const activeSlider = sliders?.data.filter((item:any) => item.status === 1 && item.type === 1)[0]
  ...
  if (isSuccess) {
    content = (
      <Box>
          ...
          <Typography component='h1' variant='h4' >اسلایدر فعال: </Typography>
          <Typography component='h1' variant='h4' color='success' >{activeSlider?.title}</Typography>
        </Box>
        <DataGrid
          autoHeight
          rows={sliders?.data}
